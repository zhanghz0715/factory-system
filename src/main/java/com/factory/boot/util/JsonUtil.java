...
public class JsonUtil {
	 ...
	 * @param pojoClass  要转成的class，必须是public的
	 * @param map        集合类型中的元素类型， key为属性名称，value为类型
	 * @return
	 */
	 ...
	 * 把java对象转换成json字符串，object中的集合和自定义类型不会转换
	 * 
	 * @param object            需要转为json的java pojo对象
	 * @param includeProperties 包含的需要附加转换的属性，一般是自定义类型
	 * @return
	 */
	 ...
	 * 
	 * @param collections       需要转为json的集合，集合元素只能是普通的pojo类，不能是集合、map、数组
	 * @param includeProperties 集合中元素需要转换的属性，默认只转换8种基本类型和String类型，集合类型不转换
	 * @return
	 */
	...
	private static boolean isBaseType(Class<?> type) {
		...
		if (type == Integer.class || type == Boolean.class || type == Character.class || type == Byte.class
				|| type == Short.class || type == Long.class || type == Float.class || type == Double.class) {
			return true;
		}
		return false;
	}
