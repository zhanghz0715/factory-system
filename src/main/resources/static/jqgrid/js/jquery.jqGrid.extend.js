	...
	var DataGrid = function(options, $this) {
			var dataGrid = typeof $this != "undefined" ? $this : options.dataGrid ? options.dataGrid : $("#dataGrid"),
				...
				options = $.extend({
                    ...
                    inputCurrent: $("#current", searchForm),
					inputPageSize: $("#size", searchForm),
					inputOrderBy: $("#orderBy", searchForm),
                    inputAsc: $("#asc", searchForm),
					beforeRequest: function(data) {
						...
						case "script":
							if (options.url == undefined || options.url == "") {
								modals.info("设置searchForm表单指定错误或URL为空。");
								return false
							}
						}
						//js.loading();
					...
					loadError: function(data) {
						...
						$(".btn").attr("disabled", false);
						if (data.responseText.startWith("<!--login_page_identity-->")) {
							modals.confirm("会话超时, 请重新登录!", function (){
								window.top.location= document.URL;
							});
						} else {
							modals.error("数据加载失败, 请联系管理员或者重试!");
						}
					...
					onSortCol: function(index, iCol, sortorder) {
						if (options.inputOrderBy && options.inputOrderBy.length) {
                            ...
                            options.inputAsc.val(sortorder == "asc"? true: false);
						}
					},
							...
							if ($(this).attr("data-click-binded") == undefined) {
								...
								$(this).click(function() {
									...
									if (confirm != undefined) {
										...
										modals.confirm(confirm, function(data) {
							...
							if ($(this).attr("data-mouseover-binded") == undefined) {
								...
								elements.mouseover(function() {
									var bdiv = dataGrid.closest(".ui-jqgrid-bdiv"),
                                        ...
                                        top = pos.top - items.height() / 2 + 12,
                                        left = pos.left - items.width() + 46;
			...
			if (options.inputCurrent.length == 0) {
				var current = searchForm.data("current");
				searchForm.append('<input id="current" name="current" type="hidden" value="' + (current ? current : "1") + '"/>');
				options.inputCurrent = $("#current", searchForm)
			}
			if (options.inputPageSize.length == 0) {
				var pageSize = searchForm.data("size");
				searchForm.append('<input id="size" name="size" type="hidden" value="' + (pageSize ? pageSize : "10") + '"/>');
				options.inputPageSize = $("#size", searchForm)
			}
			if (options.inputOrderBy.length == 0) {
				var orderBy = searchForm.data("orderByField");
				searchForm.append('<input id="orderByField" name="orderByField" type="hidden" value="' + (orderBy ? orderBy : "") + '"/>');
				options.inputOrderBy = $("#orderByField", searchForm)
			}
			if(options.inputAsc.length == 0){
                var asc =  searchForm.data("asc");
                searchForm.append('<input id="asc" name="asc" type="hidden" value="' + (asc ? asc : "true") + '"/>');
                options.inputAsc =  $("#asc", searchForm);
            }
...
//					}
					//暂时修改 by.lyy
					var modal = $(dataGrid).parents(".modal-body");
					//var tabContent = $(dataGrid).parents(".tab-content");
					if(!modal.length > 0){
						var headerHeight = $(dataGrid).parents(".ui-jqgrid").find(".ui-jqgrid-hdiv").height();
						var pageHeight = 0;
						if(dataGridPage.length > 0){
							pageHeight = '46';
						}
						gridHeight = $(window).height() - headerHeight - 119 - pageHeight-options.autoGridHeightFix;
					}else{
						//弹框中设置高度
						var modalHeight = modal.height();
						var box = $(dataGrid).parents(".box-body");
						var boxHead = 20;
						if(box.siblings(".box-header").length>0){
							boxHead = boxHead + box.siblings(".box-header").height() + 1;
						}
						if(box.siblings(".box-footer").length>0){
							boxHead = boxHead + box.siblings(".box-footer").height() +20;
						}
						var headerHeight = $(dataGrid).parents(".ui-jqgrid").find(".ui-jqgrid-hdiv").height();
						var pageHeight = 0;
						if(dataGridPage.length > 0){
						...
						gridHeight = modalHeight - boxHead - pageHeight - headerHeight-options.autoGridHeightFix;
					}
			...
			function onSelectRow(rowid, status) {
                return dataGrid.jqGrid("onSelectRow",rowid,status);
            }
            function onSelectAll(aRowids, status) {
                return dataGrid.jqGrid("onSelectAll",aRowids,status);
            }
            function gridComplete() {
                return dataGrid.jqGrid("gridComplete");
            }
            function setSelection(ids) {
                return dataGrid.jqGrid("setSelection",ids)
            }
			function getDataIDs() {
			...
			function refresh(pageNo, pageSize) {
				if (pageNo) {
					options.inputCurrent.val(pageNo)
				}
				if (pageSize) {
					options.inputPageSize.val(pageSize)
				}
				var params = {};
				if (options.inputCurrent.val() != "") {
					params.page = options.inputCurrent.val()
				}
			...
			function expandTreeNode(level) {
				...
				setTimeout(function() {
					...
					if (dataGrid.expandNodeIds.length > 100) {
						modals.info($.jgrid.format($.jgrid.extend.expandTooMany, dataGrid.expandNodeIds.length));
						dataGrid.expandNodeIds = []
					} else {
						$("#" + dataGrid.expandNodeIds.shift() + ":visible .tree-plus", dataGrid).click()
					}
			...
			var self = {
                ...
                onSelectRow:function(rowid, status){
				    return onSelectRow(rowid, status)
                },
                onSelectAll:function(aRowids, status){
				    return onSelectAll(aRowids, status)
                },
                gridComplete:function(){
				    return gridComplete()
                },
...
function pageHtml(c) {
    ...
    //上一页
	if (!c.prev) {
		c.prev = c.current == 1? 1: c.current -1
	}
	//下一页
	if (!c.next) {
		c.next = c.current == c.pages? c.pages:c.current + 1
	}
	...
	c.bothNum = 1;
	c.centerBegin = c.current - parseInt(c.centerNum / 2);
	...
	b.push('<input type="text" value="' + c.current + '" onkeypress="var e=window.event||event;var c=e.keyCode||e.which;if(c==13)');
	b.push(c.funcName + "(this.value," + c.size + ",'" + c.funcParam + '\');" onclick="this.select();"/> ' + $.jgrid.extend.pageLabelB + " ");
	b.push('<input type="text" value="' + c.size + '" onkeypress="var e=window.event||event;var c=e.keyCode||e.which;if(c==13)');
	b.push(c.funcName + "(" + c.current + ",this.value,'" + c.funcParam + '\');" onclick="this.select();"/> ');
	b.push($.jgrid.format($.jgrid.extend.pageLabelC, c.total));
	...
	b.push('</ul>\n<ul class="pagination">\n');
	if (c.current == c.last) {
        b.push('<li class="paginate_button page-item disabled"><a href="javascript:" class="page-link"><i class="fa fa-angle-right"></i></a></li>\n')
    } else {
        b.push('<li class="paginate_button page-item "><a href="javascript:"  class="page-link" onclick="' + c.funcName + "(" + c.next + "," + c.size + ",'" + c.funcParam + '\');"><i class="fa fa-angle-right"></i></a></li>\n')
    }
	
	
    for (a = c.first; a < c.first + c.bothNum && a < c.centerBegin; a++) {
    	firstStr = '<li class="paginate_button page-item "><a href="javascript:" onclick="'+ c.funcName + "(" + a + "," + c.size + ",'" + c.funcParam + '\');" class="page-link">'+(a + 1 - c.first)+'</a></li>';
        //b.push('<li class="paginate_button page-item "><a href="javascript:" onclick="'+ c.funcName + "(" + a + "," + c.size + ",'" + c.funcParam + '\');" class="page-link">'+(a + 1 - c.first)+'</a></li>');
    }

    if(a < c.centerBegin){
    	minStr = '<li class="paginate_button page-item disabled"><a href="javascript:" class="page-link">...</a></li>\n';
        //b.push('<li class="paginate_button page-item disabled"><a href="javascript:" class="page-link">...</a></li>\n')
    } else {
        c.centerEnd++
    }
    ...
    for (a = c.centerEnd; a >= c.centerBegin; a--) {
        if (a == c.current) {
        	mainStr.push('<li class="paginate_button page-item active"><a href="javascript:" class="page-link">' + (a + 1 - c.first) + "</a></li>\n")
            //b.push('<li class="paginate_button page-item active"><a href="javascript:" class="page-link">' + (a + 1 - c.first) + "</a></li>\n")
        } else {
        	mainStr.push('<li class="paginate_button page-item "><a href="javascript:" class="page-link" onclick="' + c.funcName + "(" + a + "," + c.size + ",'" + c.funcParam + "');\">" + (a + 1 - c.first) + "</a></li>\n")
            //b.push('<li class="paginate_button page-item "><a href="javascript:" class="page-link" onclick="' + c.funcName + "(" + a + "," + c.size + ",'" + c.funcParam + "');\">" + (a + 1 - c.first) + "</a></li>\n")
        }
    }
   /* for (a = c.centerBegin; a <= c.centerEnd; a++) {
    ...
    if (c.last - c.centerEnd > c.bothNum) {
    	maxStr = '<li class="paginate_button page-item disabled"><a href="javascript:" class="page-link">...</a></li>\n';
        //b.push('<li class="paginate_button page-item disabled"><a href="javascript:" class="page-link">...</a></li>\n');
        c.centerEnd = c.last - c.bothNum
    }
    for (a = c.centerEnd + 1; a <= c.last; a++) {
    	lastStr = '<li class="paginate_button page-item "><a href="javascript:" class="page-link" onclick="' + c.funcName + "(" + a + "," + c.size + ",'" + c.funcParam + "');\">" + (a + 1 - c.first) + "</a></li>\n";
        //b.push('<li class="paginate_button page-item "><a href="javascript:" class="page-link" onclick="' + c.funcName + "(" + a + "," + c.size + ",'" + c.funcParam + "');\">" + (a + 1 - c.first) + "</a></li>\n")
    }
    //从大到小将串push进去
    b.push(lastStr);
    ...
    if(c.current == c.first){
	    b.push('<li class="paginate_button page-item previous disabled"><a href="javascript:" class="page-link"><i class="fa fa-angle-left"></i></a></li>');
    }else {
        b.push('<li class="paginate_button page-item previous"><a href="javascript:" onclick="' + c.funcName + "(" + c.prev + "," + c.size + ",'" + c.funcParam + '\')" class="page-link"><i class="fa fa-angle-left"></i></a></li>');
    }
	b.push("</ul>\n");
	...
	a.jgrid.extend({
		setComplexHeaders: function(b) {
			...
			return this.each(function() {
				...
				var K = this,
					...
					G, E, n, J, k, d, F, x, M, u, c, C, r = 0,
					H = 0,
					A = false,
					B, v, w, f, y = K.p.colModel,
					D = y.length,
					t = K.grid.headers,
					...
					e = h.children("thead"),
					s, q, o = h.find(".jqg-first-row-header");
				if (o[0] === undefined) {
					o = a("<tr>", {
						role: "row",
						"aria-hidden": "true"
		...
		createFrozenHtable: function(e, g) {
			var P = this[0],
				...
				L = 0,
				C = false,
				K, G, z, d, J, y, B, R, l, D, u, b, E, v, w, h, c = P.grid.fhDiv.height(),
